...
async function resolveInfisicalSecret(
  ...
  if (!fetcher) {
    if (cached) {
      console.warn(
        `Infisical fetcher unavailable; using cached secret for ${placeholder.path}:${placeholder.key}.`
      );
      return cached;
    }
    ...
    console.warn(
      `Infisical fetcher unavailable; returning ${fallback === undefined ? "undefined" : "default"} for ${placeholder.path}:${placeholder.key}.`
    );
  ...
  const fetchPromise = fetcher({
    ...
    if (cached) {
      console.warn(
        `Infisical fetch failed (${error instanceof Error ? error.message : String(error)}); using cached secret for ${placeholder.path}:${placeholder.key}.`
      );
      return cached;
    }
    ...
    console.warn(
      `Infisical fetch failed (${error instanceof Error ? error.message : String(error)}); returning ${fallback === undefined ? "undefined" : "default"} for ${placeholder.path}:${placeholder.key}.`
    );

    if (fallback === undefined && !placeholder.optional) {
      options.onMissingSecret?.(placeholder, "infisical");
      throw new Error(
        `Failed to fetch Infisical secret ${placeholder.path}:${placeholder.key}: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
  ...
  const guardedPromise = fetchPromise.then(
    value => value,
    error => {
      if (useCache) {
